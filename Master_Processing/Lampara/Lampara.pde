...
float rtod = 57.2957795; //elements for calculate the kinematic

//Parameter of Length
float l1 = 150; //Joint 1
float l2 = 200; //Joint 2
float l3 = 160; //Joint 3
...
{ 
  ...
  //String portName = "";
  myPort = new Serial(this, "COM5", 9600); //Baudrat default 9600, COM of Master
  myPort.bufferUntil('\n');
...
int Arm(float x, float y, float z, float g) 
{
  ...
  // Set value
  float Base = theta1*rtod;
  float Shoulder = theta2*rtod;
  float Elbow = theta3*rtod;
  while ((int)Elbow <= 0 || (int)Shoulder <= 0)
  {
    return 1;
  }
  slider1.setValue(180 - Base);
  slider2.setValue(Shoulder);
