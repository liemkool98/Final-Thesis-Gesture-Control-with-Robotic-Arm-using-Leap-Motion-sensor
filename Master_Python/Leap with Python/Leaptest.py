...
import Leap, threadarom Leap import CircleGesture, KeyTapGesture, ScreenTapGesture, SwipeGesture
from pyduino import *
import serial
...
class SampleListener(Leap.Listener):
    ...
    SERVO_PIN = 6 # Azimuthal Servo motor pin
    #global AZIMUTHAL_LIMIT = 180 # we want our motor to go between 0 and 180
    ...
    def on_exit(self, controller):
        ...
        #self.a.close()
        self.a.serial.write(chr(SERVO_PIN))
        self.a.serial.write(chr(90))
    ...
    def on_frame(self, controller):
        ...
        if self.newtime-self.oldtime > 0.1: # if difference between times is 10ms
            ...
            # write the value to servo on arduino via serial connection
            #self.a.servo_write(self.SERVO_PIN,int(XPOS_servo)) # turn LED on
            self.a.Servo.write(chr(self.SERVO_PIN))
            self.a.serial.write(chr(self.SERVO_PIN))
            self.a.serial.write(chr(int(XPOS_servo)))
            # update the old time
